...
### 2026-08-16（同日）反作弊转向「服务端权威重算」+ 止血

**背景**：玩家反馈「养出 7000亿亿（7e19）没收录」。诊断确认：rate_cap 反复调整（1e12→1e13→1e14）是打地鼠——游戏数值指数增长（连点器×狂暴×三通道），固定墙永远追不上真实玩家，而作弊者又能伪造任意分数。**决策：停止调墙，转向方案 A「服务端权威重算」**（云端用移植的产能公式自己算分数，客户端 total 仅供参考）。

**两步走落地**：
1. **第一步（止血，已完成）**：rate_cap 提到 **1e17/s**——正常玩家满配峰值 ~2.8e13/s，1e17 有 3500 倍裕量，**任何正常玩家都不可能撞墙**（7000亿亿=7e19 的玩家，账号存在 12 分钟以上即通过）。作弊者仍受 created_at 锚点 + 只增不减 + 绝对上限 1e24 约束。
2. **第二步（方案 A，验证版部署中）**：`docs/server-authoritative-schema.sql` 已写好：
   - 新表 `players_state`（服务端权威存档：total_server/count_server/pool_cost_paid/last_seen）
   - RPC `recalc_score(p_state)`：服务端用移植公式算秒产（进化×商店×节日×加速×狂暴），
     按时间差累加权威 total，写 scores 供榜单读；含「买得起」校验（虚报池子数回退上次权威存档）
   - 防作弊点：客户端虚报 total 无效（服务端只认自己的增量）；虚报秒产/池子被成本校验拦；
     改加速/狂暴时间戳被「新增时长窗口」判定拦；离线刷时长单次封顶 8h
   - **待办**：验证版先用真实账号对比「云端算的 vs 客户端实际」确认公式一致，再全量上线；
     前端需改上报 state + 用权威 total 展示（另开迭代）

**风险说明**：服务端公式与 core.js 数值不一致会「算错」分数（比误伤更严重），故必须验证版先行。

---

### 2026-08-16（同日）文案更名：仙界 → 封神榜（v0.71）

**变更**：运营将玩家可见文案「仙界」统一改为「封神榜」，对应「破碎虚空飞升」改为「登封神榜」。纯文案调整，**不影响任何技术机制**：
- 前端 tab：「🐉 仙界」→「🐉 封神榜」
- 空榜提示/我的排名文案：「飞升仙界」→「登封神榜」
- 技术标识**不变**：`leaderboard_immortal` 视图名、`IMMORTAL_THRESHOLD` 常量、`lbFaction='immortal'`（避免改后端）
- 版本号：v0.7 → v0.71

**注意**：历史日志中「仙界」字样为当时命名，保留原貌；后续文档一律用「封神榜」。
